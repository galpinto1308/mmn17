...
void menu()
{
    ...
    // Add sliders to control the ambient light and lamp intensity
    ...
    // Add a checkbox to toggle the robot view
...
void myDisplay() {
    ...
    if (robot_view) {
        ...
        gluLookAt(robot.pos[0] + camera_distance * sin(camera_angle_y * PI / 180.0f) * cos(camera_angle_x * PI / 180.0f),
                  robot.pos[1] + camera_distance * sin(camera_angle_x * PI / 180.0f),
                  robot.pos[2] + camera_distance * cos(camera_angle_y * PI / 180.0f) * cos(camera_angle_x * PI / 180.0f),
...
void keyPressed(int key, int x, int y) {
    switch (key) {
        case GLUT_KEY_UP:
            if (lamp_flag || lamp_dir_flag) {
                if (lamp_flag) lamp_position[1] += 0.5f;
                lamp_direction[1] += 0.5f;
            } else if (lamp_int_flag) {
                lamp_intensity += 0.05f;
            } else if (move_flag == 9) {
                if (ambient_factor <= 1.0f) ambient_factor += 0.05f;
            } else if (move_flag == 0) {
                camera_angle_x += 5.0f;
            }
            break;
        case GLUT_KEY_DOWN:
            if (lamp_flag || lamp_dir_flag) {
                ...
                if (ambient_factor >= 0.0f) ambient_factor -= 0.05f;
            } else if (move_flag == 0) {
                camera_angle_x -= 5.0f;
            }
            break;
        case GLUT_KEY_LEFT:
            if (lamp_flag || lamp_dir_flag) {
                if (lamp_flag) lamp_position[0] -= 0.5f;
                lamp_direction[0] -= 0.5f;
            } else if (move_flag == 0) {
                camera_angle_y -= 5.0f;
            } else if (move_flag == 1) {
                robot.head_angle -= 5.0f;
            }
            break;
        case GLUT_KEY_RIGHT:
            if (lamp_flag || lamp_dir_flag) {
    ...
    camera_angle_x = clamp(camera_angle_x, -10.0f, 90.0f);
    camera_angle_y = fmod(camera_angle_y, 360.0f);
...
void updateMove(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'p':
            if (lamp_flag || lamp_dir_flag) {
                if (lamp_flag) lamp_position[2] += 0.5f;
                lamp_direction[2] += 0.5f;
            }
            break;
        case 'o':
            if (lamp_flag || lamp_dir_flag) {
...
void updateVelocity() {
    robot.velocity[0] = (keys['a'] ? robot.speed : 0) + (keys['d'] ? -robot.speed : 0);
    robot.velocity[2] = (keys['w'] ? robot.speed : 0) + (keys['s'] ? -robot.speed : 0);
}
...
void timer(int value) 
{
    ...
    if (!new_robot_bb.intersects(table_bb) && !new_robot_bb.intersects(trash_can_bb) &&
        ...
        if (robot.velocity[0] != 0 || robot.velocity[2] != 0) {
            robot.rotation_angle_all = atan2(robot.velocity[0], robot.velocity[2]) * 180.0f / PI;
        }
    }
